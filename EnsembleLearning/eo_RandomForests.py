...
def ID3(S, attributes, branch_value, depth, max_depth, size):
    ...
    G = []
    for _ in range(size):
        G.append(attributes[r.randint(0, len(attributes)-1)])

    A = best_split_attribute(S, G)
    ...
    if attribute_types[A] == "categorical":
        for v in attribute_values[A]:
            ...
            if Sv.shape[0] == 0:
                ...
                root.children.append(ID3(Sv, [a for a in attributes if a != A], v, depth + 1, max_depth,size))
...
def bagged_trees(S, columns, size):
    m = S.shape[0]
    samples = []
    for _ in range(m):
        i = r.randint(0, m-1)
    ...
    trained_trees.append(ID3(XY_t, columns, None, 0, float('inf'), size))
...
def decompostion(train, test):
    ...
    for i in range(100):
        ...
        for _ in range(500):
            bagged_trees(sample_data, columns[:-1], 4)
    ...
    for row in test.itertuples(index=False):
        ...
        p = []
        for i in range(100):
            p.append(1 if bagged_predict_row(bagged_predictors[i], row) == "yes" else 0)
...
def main():
    ...
    for size in feature_subset_sizes:
        ...
        for T in range(1, 500):
            bagged_trees(train, columns[:-1], size)
